// Ignore Spelling: AES

namespace AES
{
    using System;

    public class AES
    {
        private const int Nb = 4; // Number of columns (32-bit words) comprising the State
        private const int Nk = 4; // Key length in 32-bit words
        private const int Nr = 10; // Number of rounds
        private static readonly byte[] SBox =
        [
        // AES S-box values
            
        ];
        private static readonly byte[] InvSBox =
        [
            
        ];
        private static readonly byte[] Rcon = new byte[11] { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 };

        private static byte[] RoundKeys = [];
        
        public AES(byte[] key)
        {
            if (key.Length != 16) throw new ArgumentException("Only 128-bit keys are supported.");
            RoundKeys = KeyExpansion(key);
        }

        public byte[] Encrypt(byte[] input)
        {
            byte[] state = (byte[])input.Clone();
            AddRoundKey(state, 0);

            for (int round = 1; round < Nr; round++)
            {
                SubBytes(state);
                ShiftRows(state);
                MixColumns(state);
                AddRoundKey(state, round);
            }

            SubBytes(state);
            ShiftRows(state);
            AddRoundKey(state, Nr);

            return state;
        }

        public byte[] Decrypt(byte[] input)
        {
            byte[] state = (byte[])input.Clone();
            AddRoundKey(state, Nr);

            for (int round = Nr - 1; round > 0; round--)
            {
                InvShiftRows(state);
                InvSubBytes(state);
                AddRoundKey(state, round);
                InvMixColumns(state);
            }

            InvShiftRows(state);
            InvSubBytes(state);
            AddRoundKey(state, 0);

            return state;
        }

        // ==== Core Steps ====

        private static void SubBytes(byte[] state)
        {
            for (int i = 0; i < state.Length; i++)
                state[i] = SBox[state[i]];
        }

        private static void InvSubBytes(byte[] state)
        {
            for (int i = 0; i < state.Length; i++)
                state[i] = InvSBox[state[i]];
        }

        private static void ShiftRows(byte[] s)
        {
            byte temp;

            temp = s[1]; s[1] = s[5]; s[5] = s[9]; s[9] = s[13]; s[13] = temp;
            temp = s[2]; s[2] = s[10]; s[10] = temp;
            temp = s[6]; s[6] = s[14]; s[14] = temp;
            temp = s[3]; s[3] = s[15]; s[15] = s[11]; s[11] = s[7]; s[7] = temp;
        }

        private static void InvShiftRows(byte[] s)
        {
            byte temp;

            temp = s[13]; s[13] = s[9]; s[9] = s[5]; s[5] = s[1]; s[1] = temp;
            temp = s[2]; s[2] = s[10]; s[10] = temp;
            temp = s[6]; s[6] = s[14]; s[14] = temp;
            temp = s[7]; s[7] = s[11]; s[11] = s[15]; s[15] = s[3]; s[3] = temp;
        }

        private static void MixColumns(byte[] s)
        {
            for (int c = 0; c < 4; c++)
            {
                int i = c * 4;
                byte a = s[i], b = s[i + 1], c1 = s[i + 2], d = s[i + 3];

                s[i] = (byte)(Gmul(a, 2) ^ Gmul(b, 3) ^ c1 ^ d);
                s[i + 1] = (byte)(a ^ Gmul(b, 2) ^ Gmul(c1, 3) ^ d);
                s[i + 2] = (byte)(a ^ b ^ Gmul(c1, 2) ^ Gmul(d, 3));
                s[i + 3] = (byte)(Gmul(a, 3) ^ b ^ c1 ^ Gmul(d, 2));
            }
        }

        private static void InvMixColumns(byte[] s)
        {
            for (int c = 0; c < 4; c++)
            {
                int i = c * 4;
                byte a = s[i], b = s[i + 1], c1 = s[i + 2], d = s[i + 3];

                s[i] = (byte)(Gmul(a, 0x0e) ^ Gmul(b, 0x0b) ^ Gmul(c1, 0x0d) ^ Gmul(d, 0x09));
                s[i + 1] = (byte)(Gmul(a, 0x09) ^ Gmul(b, 0x0e) ^ Gmul(c1, 0x0b) ^ Gmul(d, 0x0d));
                s[i + 2] = (byte)(Gmul(a, 0x0d) ^ Gmul(b, 0x09) ^ Gmul(c1, 0x0e) ^ Gmul(d, 0x0b));
                s[i + 3] = (byte)(Gmul(a, 0x0b) ^ Gmul(b, 0x0d) ^ Gmul(c1, 0x09) ^ Gmul(d, 0x0e));
            }
        }

        private static void AddRoundKey(byte[] state, int round)
        {
            for (int i = 0; i < 16; i++)
                state[i] ^= RoundKeys[round * 16 + i];
        }

        // ==== Key Expansion ====

        private static byte[] KeyExpansion(byte[] key)
        {
            byte[] expanded = new byte[176]; // 11 round keys of 16 bytes
            Array.Copy(key, expanded, 16);

            byte[] temp = new byte[4];
            int bytesGenerated = 16;
            int rconIndex = 1;

            while (bytesGenerated < 176)
            {
                Array.Copy(expanded, bytesGenerated - 4, temp, 0, 4);

                if (bytesGenerated % 16 == 0)
                {
                    RotWord(temp);
                    SubWord(temp);
                    temp[0] ^= Rcon[rconIndex++];
                }

                for (int i = 0; i < 4; i++)
                    expanded[bytesGenerated++] = (byte)(expanded[bytesGenerated - 16] ^ temp[i]);
            }

            return expanded;
        }

        private static void RotWord(byte[] word)
        {
            byte temp = word[0];
            word[0] = word[1];
            word[1] = word[2];
            word[2] = word[3];
            word[3] = temp;
        }

        private static void SubWord(byte[] word)
        {
            for (int i = 0; i < 4; i++)
                word[i] = SBox[word[i]];
        }

        // GF(2^8) multiplication
        private static byte Gmul(byte a, byte b)
        {
            byte p = 0;
            for (int i = 0; i < 8; i++)
            {
                if ((b & 1) != 0) p ^= a;
                bool hiBitSet = (a & 0x80) != 0;
                a <<= 1;
                if (hiBitSet) a ^= 0x1b;
                b >>= 1;
            }
            return p;
        }
    }

}
